...
execute as @s[scores={bleeding_check=0}] run damage @s 1 dry_out
scoreboard players operation @s bleeding_check = @s bleeding
scoreboard players operation @s bleeding_check %= @s 20
execute as @s[scores={bleeding_check=0, language=0}] run tellraw @s ["",{"text":"you are now bleeding. ","color":"dark_red"},{"score":{"name":"@s","objective":"bleeding_sec"},"color":"dark_red"},{"text":"s left","color":"dark_red"}]
execute as @s[scores={bleeding_check=0, language=1}] run tellraw @s ["",{"text":"출혈 주의! ","color":"dark_red"},{"score":{"name":"@s","objective":"bleeding_sec"},"color":"dark_red"},{"text":"초 남음","color":"dark_red"}]
